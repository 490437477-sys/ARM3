# -*- coding: utf-8 -*-
"""
Система управления манипулятором — графический интерфейс на Python (русская локализация)
=========================================================================================

Расширенный графический интерфейс для 5-сервоприводного манипулятора на Arduino UNO R3.

Локализация: интерфейс, журналы и подписи сервоприводов переведены на русский язык.
Протокол последовательного порта не изменён, чтобы оставаться совместимым с прошивкой.

Протокол последовательного порта (9600 бод, конец строки \\n):
    h | help        Показать справку на стороне Arduino
    s | status      Показать текущие углы всех 5 сервоприводов
    <угол>          Установить ВСЕ сервоприводы на <угол> (S4 ограничен 0-90)
    <id> <угол>     Установить один сервопривод (0 <= id <= 4)
    a b c d e       Пакетная установка всех 5 целей сразу

Пределы: S0-S3 — 0-180, S4 (захват) — 0-90.
Примечание: S2 инвертирован внутри прошивки Arduino.

Важно: в командах <угол> — число в градусах (цифры, а не слово).
"""
...
class ServoControlApp:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title(f"Управление манипулятором  v{__version__}")
        ...
        self.traj_status       = tk.StringVar(value="\u25CF  Idle (0 кадров)")
        self.traj_list         = None
        ...
        self._log("автообновление схемы запущено (каждые " + str(ARM_REFRESH_MS) + " мс)", "info")
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Управление манипулятором",
    ...
    def _build_connection_bar(self):
        ...
        tk.Label(inner, text="Подключение", font=("Segoe UI", 10, "bold"),
                 fg=ACCENT, bg=PANEL_BG).pack(side="left", padx=(0, 12))

        tk.Label(inner, text="Порт:", fg=TEXT, bg=PANEL_BG).pack(side="left")
        ...
        tk.Label(inner, text="  Скорость:", fg=TEXT, bg=PANEL_BG).pack(side="left", padx=(8, 0))
        ...
        self.conn_btn = tk.Button(inner, text="●  Подключиться", font=("Segoe UI", 10, "bold"),
        ...
        self._status_lbl = tk.Label(inner, text="Отключено", fg=SUBTEXT, bg=PANEL_BG, font=("Segoe UI", 9))
        self._status_lbl.pack(side="left")

    def _build_servo_panel(self, parent):
        panel = ttk.LabelFrame(parent, text="  Управление сервоприводами  ", padding=(14, 10))
        panel.pack(fill="x", pady=(0, 10))

        hdr = tk.Frame(panel, bg=PANEL_BG)
        hdr.pack(fill="x", pady=(0, 4))
        tk.Label(hdr, text="", width=22, bg=PANEL_BG).pack(side="left")
        tk.Label(hdr, text="Факт", font=("Segoe UI", 9, "bold"),
                 fg=GREEN, bg=PANEL_BG, width=6).pack(side="left")
        tk.Label(hdr, text="Цель (ползунок / ввод)",
    ...
    def _build_servo_row(self, parent, idx):
        ...
        tk.Button(row, text="Уст", font=("Segoe UI", 11, "bold"),
    ...
    def _build_log_panel(self, parent):
        panel = ttk.LabelFrame(parent, text="  Журнал порта  ", padding=(10, 8))
        panel.pack(fill="x")
        ...
        # is truncated to "Cl".
        ttk.Button(toolbar, text="Копировать",  style="Small.TButton", command=self._copy_log).pack(side="right", padx=(4, 0))
        ttk.Button(toolbar, text="Очистить", style="Small.TButton", command=self._clear_log).pack(side="right")

        tk.Checkbutton(toolbar, text="Автопрокрутка", variable=self.autoscroll,
                       font=("Segoe UI", 9), fg=SUBTEXT, bg=PANEL_BG,
                       selectcolor=INPUT_BG, activebackground=PANEL_BG).pack(side="left")
        tk.Label(toolbar, text="   \u2191 отправлено (TX)   \u00b7   \u2193 получено (RX)",
        ...
        self._log("Интерфейс готов. Нажмите «\u25cf Подключиться», чтобы открыть COM-порт.", "info")
    ...
    def _build_right_panel(self, parent):
        ...
        panel = ttk.LabelFrame(parent, text="  Своя команда  ", padding=(14, 10))
        panel.pack(fill="x", pady=(0, 10))
        ...
        tk.Button(cmd_frame, text="Отправить", font=("Segoe UI", 9, "bold"),
                  bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
        ...
        tk.Label(panel, text="Пределы", font=("Segoe UI", 10, "bold"),
                 fg=ACCENT, bg=PANEL_BG).pack(anchor="w")
        tk.Label(panel, text="S0–S3: 0–180°\nS4 Захват: 0–90°\nS2 направление инвертировано в Arduino",
    ...
    def _build_arm_diagram(self, parent):
        ...
        """
        panel = ttk.LabelFrame(parent, text="  Схема состояния манипулятора  ", padding=(10, 8))
        ...
        inner.pack(fill="x")
        tk.Label(inner, text="Управление сервоприводами", font=("Segoe UI", 11, "bold"),
        ...
        self._reset_all_btn = tk.Button(
            reset_row, text="↺ Сброс всех", font=("Segoe UI", 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
    ...
    def _reset_all(self):
        ...
        self._log("сброс всех сервоприводов на 90°", "info")
    ...
    def _arm_render_loop(self):
        try:
            ...
            try:
                self._log("ошибка отрисовки схемы манипулятора: " + str(exc), "err")
            except Exception:
                pass
    ...
    def _draw_arm(self):
        ...
        if self.connected:
            note = "целевые углы (онлайн · нажмите «Запрос статуса» для обновления фактических)"
        else:
            note = "целевые углы · не подключено"
    ...
    def _render_traj_trail(self, c):
        ...
        if len(кадров) > self.TRAIL_MAX_POINTS:
            step = max(1, len(кадров) // self.TRAIL_MAX_POINTS)
            frames = frames[::step]
        pts = []
        for f in frames:
            try:
    ...
    def _build_traj_panel(self, parent):
        panel = ttk.LabelFrame(parent, text="  Траектория  ", padding=(14, 10))
        ...
        self.traj_record_btn = tk.Button(
            row1, text="● Запись", font=("Segoe UI", 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
            relief="flat", padx=14, pady=6, cursor="hand2",
            command=self._toggle_record)
        self.traj_record_btn.pack(side="left")
        self.traj_play_btn = tk.Button(
            row1, text="▶ Пуск", font=("Segoe UI", 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
            relief="flat", padx=14, pady=6, cursor="hand2",
            command=self._toggle_play)
        self.traj_play_btn.pack(side="left", padx=(6, 0))
        tk.Button(row1, text="⏹ Стоп", font=("Segoe UI", 11, "bold"),
                  bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=self._stop_play).pack(side="left", padx=(6, 0))
        tk.Button(row1, text="Очистить", font=("Segoe UI", 11, "bold"),
                  bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
                  relief="flat", padx=14, pady=6, cursor="hand2",
        ...
        tk.Button(row2, text="⎘ Кадр", font=("Segoe UI", 11, "bold"),
                  bg=GREEN, fg="#1e1e2e", activebackground=GREEN,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=lambda: self._snapshot(force=True)).pack(side="left")
        tk.Button(row2, text="↡ Сохранить...", font=("Segoe UI", 11, "bold"),
                  bg=GREEN, fg="#1e1e2e", activebackground=GREEN,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=self._save_trajectory).pack(side="left", padx=(6, 0))
        tk.Button(row2, text="↢ Загрузить...", font=("Segoe UI", 11, "bold"),
                  bg=GREEN, fg="#1e1e2e", activebackground=GREEN,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=self._load_trajectory).pack(side="left", padx=(6, 0))
        tk.Button(row2, text="Загрузить пример", font=("Segoe UI", 11, "bold"),
        ...
        tk.Label(row3, text="Скорость:", font=("Segoe UI", 9),
                 fg=SUBTEXT, bg=PANEL_BG).pack(side="left")
        ...
        tk.Label(row3, text="мс/кадр", font=("Segoe UI", 8),
                 fg=SUBTEXT, bg=PANEL_BG).pack(side="left", padx=(4, 0))

        # Row 4: Loop checkbox
        row4 = tk.Frame(panel, bg=PANEL_BG); row4.pack(fill="x", pady=(4, 0))
        tk.Checkbutton(row4, text="Зациклить воспроизведение", variable=self.traj_loop,
    ...
    def _toggle_record(self):
        if self.recording:
            self.recording = False
            self.traj_record_btn.configure(text="● Запись", bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT)
            self._update_traj_status()
            self._log(f"запись остановлена — {len(self.trajectory)} кадров сохранено", "info")
        else:
            self.trajectory = []
            self.recording = True
            self.traj_record_btn.configure(text="■ Стоп", fg=TEXT)
            self._refresh_traj_list()
            self._update_traj_status()
            self._snapshot(force=True)
            self._log("запись начата", "info")
    ...
    def _toggle_play(self):
        ...
        if not self.trajectory:
            self._log("нечего воспроизводить — сначала запишите или загрузите траекторию", "warn")
            return
        self.playing = True
        self._suppress_snap = True
        self._play_index = 0
        self.traj_play_btn.configure(text="⏹ Пауза", fg=YELLOW)
        self._update_traj_status()
        self._log(f"воспроизведение запущено — {len(self.trajectory)} кадров", "info")
        self._play_tick()
    ...
    def _stop_play(self):
        ...
        if self.traj_play_btn is not None:
            self.traj_play_btn.configure(text="▶ Пуск", bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT)
        self._update_traj_status()
        if was_playing:
            self._log("воспроизведение остановлено", "info")
    ...
    def _play_tick(self):
        ...
        try:
            ...
            self._log(f"ошибка воспроизведения: {exc}", "err")
            self._stop_play()
            return
        ...
        try:
            ...
            self._log(f"ошибка отправки при воспроизведении: {exc}", "err")
        try:
    ...
    def _clear_trajectory(self):
        ...
        self._refresh_traj_list()
        self._update_traj_status()
        self._log("траектория очищена", "info")
    ...
    def _save_trajectory(self):
        if not self.trajectory:
            self._log("нечего сохранять — траектория пуста", "warn")
            return
        path = filedialog.asksaveasfilename(
            defaultextension=".json",
            filetypes=[("Траектория манипулятора", "*.json"), ("Все файлы", "*.*")],
        ...
        try:
            ...
            self._log(f"сохранено {len(self.trajectory)} кадров в {path}", "info")
        except Exception as exc:
            self._log(f"ошибка сохранения: {exc}", "err")
    ...
    def _load_trajectory(self):
        ...
        try:
            ...
            self._log(f"ошибка загрузки: {exc}", "err")
            return
        frames = payload.get("frames") if isinstance(payload, dict) else None
        if not frames or not isinstance(frames, list):
            self._log("файл не является допустимой траекторией (нет массива кадров)", "err")
            return
        if self.playing:
            self._stop_play()
        cleaned = []
        for f in frames:
            ...
            try:
                ...
                continue
        if not cleaned:
            self._log("в файле нет допустимых кадров", "err")
            return
        self.trajectory = cleaned
        self._play_index = 0
        self._refresh_traj_list()
        self._update_traj_status()
        self._log(f"загружено {len(cleaned)} кадров из {path}", "info")
    ...
    def _load_example_trajectory(self):
        ...
        for f in example["frames"]:
            ...
            try:
                ...
                continue
        if not cleaned:
            self._log("во встроенном примере нет допустимых кадров", "err")
            return
        self.trajectory = cleaned
        self._play_index = 0
        self._refresh_traj_list()
        self._update_traj_status()
        self._log(f"загружен встроенный пример — {len(cleaned)} кадров", "info")
        self._toggle_play()
    ...
    def _on_traj_list_select(self, _event):
        ...
        self._log(
            "перешли к кадру " + str(idx + 1) + "/" + str(len(self.trajectory))
    ...
    def _on_traj_list_edit(self, event):
        ...
        if self.playing or self.recording:
            self._log("нельзя редактировать кадр во время воспроизведения или записи", "warn")
            return
        sel = self.traj_list.curselection()
        if not sel:
            return
        ...
        def commit(_e=None):
            try:
                ...
                try:
                    ...
                    pass
                self._log(f"кадр {idx+1} отредактирован -> {new_angles}", "info")
            except ValueError as exc:
                self._log(f"редактирование отменено: {exc}", "warn")
            finally:
                try:
                    top.destroy()
                except Exception:
    ...
    def _update_traj_status(self):
        ...
        if self.playing:
            ...
            self.traj_status.set(f"▶ Воспроизведение  {idx}/{n}")
        elif self.recording:
            self.traj_status.set(f"●  Запись  ({n} кадров)")
        else:
            self.traj_status.set(f"○  Простой  ({n} кадров)")


    def _refresh_ports(self):
        ports = [p.device for p in list_ports.comports()]
        self.port_combo["values"] = ports if ports else ["(нет портов)"]
        if ports:
    ...
    def _connect(self, port, baud):
        try:
            ...
            self.ser = None
            self._set_status(False, f"Ошибка: {e}")
            self._log(f"Ошибка подключения: {e}", "err")
            return
        ...
        self._set_status(True, f"Подключено {port} @ {baud}")
        self.conn_btn.configure(text="●  Отключиться")
        self._log(f"Подключено к {port} @ {baud}", "ok")
    ...
    def _disconnect(self):
        ...
        self.conn_btn.configure(text="●  Подключиться")
    ...
    def _handle_connection_lost_ui(self):
        if self.connected:
            self._log("Подключение потеряно.", "err")
            self._disconnect()
    ...
    def _send_command(self, cmd):
        ...
        if not self.connected or self.ser is None:
            self._log(f"[не подключено]  было бы отправлено: {cmd}", "warn")
            return
        payload = (cmd.rstrip() + "\n").encode("utf-8")
        ser = self.ser
    ...
    def _on_rx_line(self, line):
        ...
        if m:
            ...
            if "STATUS" in upper or "INIT OK" in upper:
                ...
                self._log("  ↳ целевой кадр подтверждён", "ok")
            ...
            try:
                if all(a == 90 for a in angles):
                    self._home_btn.configure(
                        text="\u2713  В исходном положении (все 90°)",
                        bg=GREEN,
                    )
                else:
                    self._home_btn.configure(
