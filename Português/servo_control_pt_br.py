# -*- coding: utf-8 -*-
"""
Sistema de Controle do Braço Servo - Interface Python v2.1
==========================================================

Interface de desktop aprimorada para o braço robótico Arduino UNO R3 com 5 servos.

Mudanças na v2.1:
- Removidos os botões de ação rápida Pegar / Soltar / Relaxar
- Adicionado um diagrama 2D do braço (Canvas) que reflete os ângulos atuais
  - Vista lateral: base, braço superior, antebraço, pulso, garra
  - Rotação da base S0 mostrada como uma pequena seta de bússola na base
  - Os cinco ângulos rotulados na cor do seu servo

Protocolo serial (9600 bauds, terminado em nova linha):
    h | help        Mostrar ajuda no lado do Arduino
    s | status      Imprimir os ângulos atuais dos 5 servos
    <ângulo>        Define TODOS os servos em <ângulo> (S4 limitado a 0-90)
    <id> <ângulo>   Define um único servo (0 <= id <= 4)
    a b c d e       Define todos os 5 servos de uma só vez em lote

Limites: S0-S3 são 0-180, S4 (garra) é 0-90.
Nota: a direção do Servo2 é invertida internamente no Arduino.
"""
...
RX_POLL_MS     = 80
ARM_REFRESH_MS = 60
# O firmware do Arduino atualiza as posições dos servos a cada moveDelay ms
# (smoothStep graus por atualização), então o *mais rápido* que o braço pode seguir uma
# sequência de reprodução é um quadro por moveDelay. Usamos 40 ms (= moveDelay do
# Arduino) como piso para que a reprodução acompanhe a velocidade real de execução
# do servo, em vez da margem conservadora de 200 ms.
# O limite inferior do controle deslizante é fixado nesse valor.
...
class ServoControlApp:
    ...
    SERVO_TARGET_RE = re.compile(r"S(\d+)\s*alvo\s*=\s*(\d+)")

    def __init__(self):
        self.root = tk.Tk()
        self.root.title(f"Controle do Braço Servo  v{__version__}")
        ...
        # Gravação / reprodução de trajetória
        self.trajectory        = []
        ...
        self.traj_status       = tk.StringVar(value="\u25CF  Ocioso (0 quadros)")
        self.traj_list         = None
        ...
        # Sempre que qualquer ângulo-alvo muda, atualiza a pré-visualização do Comando
        # Personalizado e o estado do botão Início.
        ...
        self._build_ui()
        self._refresh_ports()
        self._update_cmd_preview()    # inicia a pré-visualização e o botão Início
        self.root.after(RX_POLL_MS,     self._poll_rx_queue)
        self.root.after(ARM_REFRESH_MS, self._arm_render_loop)
        self.root.protocol("WM_DELETE_WINDOW", self._on_close)
        self._log("auto-atualização do diagrama do braço iniciada (a cada " + str(ARM_REFRESH_MS) + " ms)", "info")
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Controle do Braço Servo",
    ...
    def _build_connection_bar(self):
        ...
        tk.Label(inner, text="Conexão", font=("Segoe UI", 10, "bold"),
                 fg=ACCENT, bg=PANEL_BG).pack(side="left", padx=(0, 12))

        tk.Label(inner, text="Porta:", fg=TEXT, bg=PANEL_BG).pack(side="left")
        ...
        tk.Label(inner, text="  Taxa:", fg=TEXT, bg=PANEL_BG).pack(side="left", padx=(8, 0))
    ...
    def _build_servo_panel(self, parent):
        panel = ttk.LabelFrame(parent, text="  Controle dos Servos  ", padding=(14, 10))
        panel.pack(fill="x", pady=(0, 10))

        hdr = tk.Frame(panel, bg=PANEL_BG)
        hdr.pack(fill="x", pady=(0, 4))
        tk.Label(hdr, text="", width=22, bg=PANEL_BG).pack(side="left")
        tk.Label(hdr, text="Atual", font=("Segoe UI", 9, "bold"),
                 fg=GREEN, bg=PANEL_BG, width=6).pack(side="left")
        tk.Label(hdr, text="Alvo (controle / entrada)",
    ...
    def _build_servo_row(self, parent, idx):
        ...
        tk.Button(row, text="Definir", font=("Segoe UI", 11, "bold"),
    ...
    def _build_log_panel(self, parent):
        ...
        ttk.Button(toolbar, text="Limpar", style="Small.TButton", command=self._clear_log).pack(side="right")

        tk.Checkbutton(toolbar, text="Rolagem automática", variable=self.autoscroll,
                       font=("Segoe UI", 9), fg=SUBTEXT, bg=PANEL_BG,
                       selectcolor=INPUT_BG, activebackground=PANEL_BG).pack(side="left")
        tk.Label(toolbar, text="   \u2191 enviado (TX)   \u00b7   \u2193 recebido (RX)",
        ...
        self._log("Interface pronta. Clique em \u25cf Conectar para abrir uma porta serial.", "info")
    ...
    def _build_right_panel(self, parent):
        ...
        tk.Label(panel, text="Limites", font=("Segoe UI", 10, "bold"),
                 fg=ACCENT, bg=PANEL_BG).pack(anchor="w")
        tk.Label(panel, text="S0\u2013S3: 0\u2013180\u00b0\nS4 Garra: 0\u201390\u00b0\nDireção do S2 invertida no Arduino",
    ...
    def _build_arm_diagram(self, parent):
        ...
        """
        panel = ttk.LabelFrame(parent, text="  Diagrama de Status do Braço  ", padding=(10, 8))
        ...
        inner.pack(fill="x")
        tk.Label(inner, text="Controle dos Servos", font=("Segoe UI", 11, "bold"),
        ...
        self._reset_all_btn = tk.Button(
            reset_row, text="\u21BA Redefinir Tudo", font=("Segoe UI", 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
    ...
    def _reset_all(self):
        ...
        self._log("todos os servos redefinidos para 90°", "info")
    ...
    def _arm_render_loop(self):
        try:
            ...
            try:
                self._log("erro de renderização do diagrama do braço: " + str(exc), "err")
            except Exception:
                pass
    ...
    def _draw_arm(self):
        ...
        if self.connected:
            note = "ângulos alvo (ao vivo \u00b7 clique em Status para atualizar os valores reais)"
        else:
            note = "ângulos alvo \u00b7 não conectado"
    ...
    def _build_traj_panel(self, parent):
        panel = ttk.LabelFrame(parent, text="  Trajetória  ", padding=(14, 10))
        ...
        self.traj_record_btn = tk.Button(
            row1, text="\u25CF Gravar", font=("Segoe UI", 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
            relief="flat", padx=14, pady=6, cursor="hand2",
            command=self._toggle_record)
        self.traj_record_btn.pack(side="left")
        self.traj_play_btn = tk.Button(
            row1, text="\u25B6 Reproduzir", font=("Segoe UI", 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
            relief="flat", padx=14, pady=6, cursor="hand2",
            command=self._toggle_play)
        self.traj_play_btn.pack(side="left", padx=(6, 0))
        tk.Button(row1, text="\u23F9 Parar", font=("Segoe UI", 11, "bold"),
                  bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=self._stop_play).pack(side="left", padx=(6, 0))
        tk.Button(row1, text="Limpar", font=("Segoe UI", 11, "bold"),
                  bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
                  relief="flat", padx=14, pady=6, cursor="hand2",
        ...
        tk.Button(row2, text="\u21A1 Salvar...", font=("Segoe UI", 11, "bold"),
                  bg=GREEN, fg="#1e1e2e", activebackground=GREEN,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=self._save_trajectory).pack(side="left", padx=(6, 0))
        tk.Button(row2, text="\u21A2 Carregar...", font=("Segoe UI", 11, "bold"),
                  bg=GREEN, fg="#1e1e2e", activebackground=GREEN,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=self._load_trajectory).pack(side="left", padx=(6, 0))
        tk.Button(row2, text="Exemplo pré-definido", font=("Segoe UI", 11, "bold"),
        ...
        tk.Label(row3, text="Velocidade:", font=("Segoe UI", 9),
                 fg=SUBTEXT, bg=PANEL_BG).pack(side="left")
        ...
        tk.Label(row3, text="ms/quadro", font=("Segoe UI", 8),
                 fg=SUBTEXT, bg=PANEL_BG).pack(side="left", padx=(4, 0))

        # Row 4: Loop checkbox
        row4 = tk.Frame(panel, bg=PANEL_BG); row4.pack(fill="x", pady=(4, 0))
        tk.Checkbutton(row4, text="Reproduzir em loop", variable=self.traj_loop,
        ...
        # Clique em qualquer lugar fora da lista para limpar a seleção e
        # liberar o modo em que a trajetória "controla os servos".
        self.root.bind_all("<Button-1>", self._on_traj_list_outside_click, add="+")

    def _toggle_record(self):
        if self.recording:
            self.recording = False
            self.traj_record_btn.configure(text="\u25CF Gravar", bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT)
            self._update_traj_status()
            self._log(f"gravação parada -- {len(self.trajectory)} quadros capturados", "info")
        else:
            self.trajectory = []
            self.recording = True
            self.traj_record_btn.configure(text="\u25A0 Parar", fg=TEXT)
            self._refresh_traj_list()
            self._update_traj_status()
            self._snapshot(force=True)
            self._log("gravação iniciada", "info")
    ...
    def _toggle_play(self):
        ...
        if not self.trajectory:
            self._log("nada para reproduzir -- grave ou carregue uma trajetória primeiro", "warn")
            return
        self.playing = True
        ...
        self._update_traj_status()
        self._log(f"reprodução iniciada -- {len(self.trajectory)} quadros", "info")
        self._play_tick()
    ...
    def _stop_play(self):
        ...
        if self.traj_play_btn is not None:
            self.traj_play_btn.configure(text="\u25B6 Reproduzir", bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT)
        self._update_traj_status()
        if was_playing:
            self._log("reprodução parada", "info")
    ...
    def _play_tick(self):
        ...
        try:
            ...
            self._log(f"erro de reprodução: {exc}", "err")
            self._stop_play()
            return
        ...
        try:
            ...
            self._log(f"erro de envio na reprodução: {exc}", "err")
        try:
    ...
    def _clear_trajectory(self):
        ...
        self._refresh_traj_list()
        self._update_traj_status()
        self._log("trajetória limpa", "info")
    ...
    def _save_trajectory(self):
        if not self.trajectory:
            self._log("nada para salvar -- a trajetória está vazia", "warn")
            return
        path = filedialog.asksaveasfilename(
            defaultextension=".json",
            filetypes=[("Trajetória de servos", "*.json"), ("Todos os arquivos", "*.*")],
            title="Salvar trajetória")
        if not path:
            return
        try:
            payload = {
            ...
            self._log(f"salvos {len(self.trajectory)} quadros em {path}", "info")
        except Exception as exc:
            self._log(f"falha ao salvar: {exc}", "err")

    def _load_trajectory(self):
        path = filedialog.askopenfilename(
            filetypes=[("Trajetória de servos", "*.json"), ("Todos os arquivos", "*.*")],
            title="Carregar trajetória")
        if not path:
            return
        try:
            ...
            self._log(f"falha ao carregar: {exc}", "err")
            return
        frames = payload.get("frames") if isinstance(payload, dict) else None
        if not frames or not isinstance(frames, list):
            self._log("o arquivo não é uma trajetória válida (sem vetor de quadros)", "err")
            return
        if self.playing:
            self._stop_play()
        cleaned = []
        for f in frames:
            ...
            try:
                ...
                continue
        if not cleaned:
            self._log("nenhum quadro válido no arquivo", "err")
            return
        self.trajectory = cleaned
        self._play_index = 0
        self._refresh_traj_list()
        self._update_traj_status()
        self._log(f"carregados {len(cleaned)} quadros de {path}", "info")
    ...
    def _load_example_trajectory(self):
        ...
        for f in example["frames"]:
            ...
            try:
                ...
                continue
        if not cleaned:
            self._log("a trajetória de exemplo incluída não tem quadros válidos", "err")
            return
        self.trajectory = cleaned
        self._play_index = 0
        self._refresh_traj_list()
        self._update_traj_status()
        self._log(f"exemplo incluído carregado -- {len(cleaned)} quadros", "info")
        self._toggle_play()
    ...
    def _on_traj_list_select(self, _event):
        ...
        for i, a in enumerate(frame):
            try:
                ...
                pass
        # Envia o comando para realmente mover os servos (lida com
        # "não conectado" de forma graciosa e sincroniza target_vars + canvas).
        self._send_batch_and_sync(frame)
        self._log(
            "pulou para o quadro " + str(idx + 1) + "/" + str(len(self.trajectory))
            + ": " + " ".join(str(a) for a in frame),
            "info")

    def _on_traj_list_outside_click(self, event):
        """Clique em qualquer lugar fora da lista de trajetórias para limpar
        a seleção. Isso libera o modo em que o "quadro controla os servos"
        para que mover os controles depois não seja interpretado como
        pular entre quadros.
        """
    ...
    def _on_traj_list_edit(self, event):
        ...
        if self.playing or self.recording:
            self._log("não é possível editar o quadro durante reprodução ou gravação", "warn")
            return
        sel = self.traj_list.curselection()
        if not sel:
            return
        ...
        def commit(_e=None):
            try:
                ...
                if _e is not None and getattr(_e, "widget", None) is top:
                    return
                txt = entry.get().strip()
                # Remove o prefixo "N: " ou "N:" se o usuário o manteve
                if ':' in txt:
                    txt = txt.split(':', 1)[1].strip()
                parts = txt.split()
                if len(parts) != 5:
                    raise ValueError("esperados 5 valores de ângulo")
                new_angles = []
                ...
                try:
                    ...
                    pass
                self._log(f"quadro {idx+1} editado -> {new_angles}", "info")
            except ValueError as exc:
                self._log(f"edição cancelada: {exc}", "warn")
            finally:
                try:
                    top.destroy()
                except Exception:
    ...
    def _update_traj_status(self):
        ...
        if self.playing:
            ...
            self.traj_status.set(f"\u25B6 Reproduzindo  {idx}/{n}")
        elif self.recording:
            self.traj_status.set(f"\u25CF  Gravando  ({n} quadros)")
        else:
            self.traj_status.set(f"\u25CB  Ocioso  ({n} quadros)")


    def _refresh_ports(self):
        ports = [p.device for p in list_ports.comports()]
        self.port_combo["values"] = ports if ports else ["(sem portas)"]
        if ports:
    ...
    def _connect(self, port, baud):
        try:
            ...
            self.ser = None
            self._set_status(False, f"Falha: {e}")
            self._log(f"Falha na conexão: {e}", "err")
            return
    ...
    def _send_command(self, cmd):
        ...
        if not self.connected or self.ser is None:
            self._log(f"[não conectado]  enviaria: {cmd}", "warn")
            return
        payload = (cmd.rstrip() + "\n").encode("utf-8")
        ser = self.ser
        with self._send_lock:
            try:
                ...
                self._log(f"Falha no envio: {e}", "err")
    ...
    def _on_rx_line(self, line):
        ...
        if m:
            ...
            elif "LOTE OK ALVO" in upper:
                for i, ang in enumerate(angles):
                    self.target_vars[i].set(ang)
                self._log("  \u21b3 quadro alvo confirmado", "ok")
            ...
            try:
                if all(a == 90 for a in angles):
                    self._home_btn.configure(
                        text="\u2713  Em casa (todos a 90°)",
                        bg=GREEN,
                    )
                else:
                    self._home_btn.configure(
                        text="\u2302  Início (todos a 90°)",
                        bg=ACCENT,
                    )
            except Exception:
                pass
    ...
    def _quick_action(self, btn, action):
        """Executa uma ação rápida e faz o botão piscar para o usuário ver a
        resposta.  action em {"home", "status", "help"}.

        A ordem importa: executamos a ação PRIMEIRO para que o bg do botão
        já esteja atualizado (por ex. para "Em casa" verde) antes de
        capturá-lo como orig_bg, depois piscamos amarelo por cima e
        restauramos para o novo estado. Assim o flash fica sempre visível.
        """
        if action == "home":
