...
void setup() {
  ...
  printHelpInfo();
  Serial.print("Init OK, Posição Inicial: ");
  printNumArray(angles, SERVO_COUNT);
}

// ===================== CONVERSÃO DE INVERSÃO DE ÂNGULO DO SERVO =====================
...
void parseSerialCommand(String cmd){
  ...
  if(cmd == "s" || cmd == "status"){
    Serial.print("STATUS Todos Servos [S0,S1,S2,S3,S4]: ");
    printNumArray(angles, SERVO_COUNT);
    return;
  }
  ...
  if(valCount == 1){
    ...
    Serial.print("Todos os servos com alvo ");
  ...
  Serial.println("Erro: comando inválido, digite h para ajuda");
}

// ===================== UTILITÁRIOS DE IMPRESSÃO SERIAL =====================
void printHelpInfo(){
  Serial.println("======== Lista de Comandos do Braço Servo ========");
  Serial.println("h        Mostrar ajuda");
  Serial.println("s        Imprimir os ângulos atuais dos 5 servos");
  Serial.println("90       Define o alvo de todos os servos para 90");
  Serial.println("0 60     Define o alvo de S0 para 60");
  Serial.println("0 90 90 90 40 Define o alvo de todos os servos em lote");
  Serial.println("Limites: S0~S3 0-180 | S4(Garra) 0-90");
  Serial.println("Nota: direção do Servo2 invertida internamente");
  Serial.println("================================================");
}
