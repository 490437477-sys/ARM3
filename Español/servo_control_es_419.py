# -*- coding: utf-8 -*-
"""
Sistema de Control del Brazo Servo - Interfaz Python v2.1
==========================================================

Interfaz de escritorio mejorada para el brazo robótico Arduino UNO R3 con 5 servos.

Cambios en la v2.1:
- Eliminados los botones de acción rápida Agarrar / Soltar / Relajar
- Añadido un diagrama 2D del brazo (Canvas) que refleja los ángulos actuales
  - Vista lateral: base, brazo superior, antebrazo, muñeca, pinza
  - Rotación de la base S0 mostrada como una pequeña flecha de brújula en la base
  - Los cinco ángulos etiquetados con el color de su servo

Protocolo serial (9600 baudios, terminado en nueva línea):
    h | help        Mostrar ayuda en el lado del Arduino
    s | status      Imprimir los ángulos actuales de los 5 servos
    <ángulo>        Fija TODOS los servos en <ángulo> (S4 limitado a 0-90)
    <id> <ángulo>   Fija un único servo (0 <= id <= 4)
    a b c d e       Fija los 5 servos de una sola vez en lote

Límites: S0-S3 son 0-180, S4 (pinza) es 0-90.
Nota: la dirección del Servo2 se invierte internamente en el Arduino.
"""
...
RX_POLL_MS     = 80
ARM_REFRESH_MS = 60
# El firmware del Arduino actualiza las posiciones de los servos cada moveDelay ms
# (smoothStep grados por actualización), así que lo *más rápido* que el brazo puede seguir una
# secuencia de reproducción es un cuadro por moveDelay. Usamos 40 ms (= moveDelay del
# Arduino) como piso para que la reproducción siga la velocidad real de ejecución
# del servo, en lugar del margen conservador de 200 ms.
# El límite inferior del control deslizante se fija en este valor.
...
class ServoControlApp:
    ...
    SERVO_TARGET_RE = re.compile(r"S(\d+)\s*objetivo\s*=\s*(\d+)")

    def __init__(self):
        self.root = tk.Tk()
        self.root.title(f"Control del Brazo Servo  v{__version__}")
        ...
        # Grabación / reproducción de trayectoria
        self.trajectory        = []
        ...
        self.traj_status       = tk.StringVar(value="\u25CF  Inactivo (0 cuadros)")
        self.traj_list         = None
        ...
        # Cada vez que cambia cualquier ángulo objetivo, actualiza la vista previa del
        # Comando Personalizado y el estado del botón Inicio.
        ...
        self._build_ui()
        self._refresh_ports()
        self._update_cmd_preview()    # inicializa la vista previa y el botón Inicio
        self.root.after(RX_POLL_MS,     self._poll_rx_queue)
        self.root.after(ARM_REFRESH_MS, self._arm_render_loop)
        self.root.protocol("WM_DELETE_WINDOW", self._on_close)
        self._log("auto-actualización del diagrama del brazo iniciada (cada " + str(ARM_REFRESH_MS) + " ms)", "info")
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Control del Brazo Servo",
                 font=("Segoe UI", 20, "bold"), fg=ACCENT, bg=BG).pack(side="left")
        tk.Label(header, text="  \u00b7  Arduino UNO R3  \u00b7  5\u00d7 MG995  \u00b7  Serial @ 9600",
    ...
    def _build_connection_bar(self):
        ...
        tk.Label(inner, text="Conexión", font=("Segoe UI", 10, "bold"),
                 fg=ACCENT, bg=PANEL_BG).pack(side="left", padx=(0, 12))

        tk.Label(inner, text="Puerto:", fg=TEXT, bg=PANEL_BG).pack(side="left")
        ...
        tk.Label(inner, text="  Baudios:", fg=TEXT, bg=PANEL_BG).pack(side="left", padx=(8, 0))
        ...
        self.conn_btn = tk.Button(inner, text="\u25cf  Conectar", font=("Segoe UI", 10, "bold"),
        ...
        self._status_lbl = tk.Label(inner, text="Desconectado", fg=SUBTEXT, bg=PANEL_BG, font=("Segoe UI", 9))
        self._status_lbl.pack(side="left")

    def _build_servo_panel(self, parent):
        panel = ttk.LabelFrame(parent, text="  Control de los Servos  ", padding=(14, 10))
        panel.pack(fill="x", pady=(0, 10))

        hdr = tk.Frame(panel, bg=PANEL_BG)
        hdr.pack(fill="x", pady=(0, 4))
        tk.Label(hdr, text="", width=22, bg=PANEL_BG).pack(side="left")
        tk.Label(hdr, text="Real", font=("Segoe UI", 9, "bold"),
                 fg=GREEN, bg=PANEL_BG, width=6).pack(side="left")
        tk.Label(hdr, text="Objetivo (control / entrada)",
    ...
    def _build_servo_row(self, parent, idx):
        ...
        tk.Button(row, text="Fijar", font=("Segoe UI", 11, "bold"),
    ...
    def _build_log_panel(self, parent):
        panel = ttk.LabelFrame(parent, text="  Registro Serial  ", padding=(10, 8))
        panel.pack(fill="x")
        ...
        # is truncated to "Cl".
        ttk.Button(toolbar, text="Copiar",  style="Small.TButton", command=self._copy_log).pack(side="right", padx=(4, 0))
        ttk.Button(toolbar, text="Limpiar", style="Small.TButton", command=self._clear_log).pack(side="right")

        tk.Checkbutton(toolbar, text="Desplazamiento automático", variable=self.autoscroll,
                       font=("Segoe UI", 9), fg=SUBTEXT, bg=PANEL_BG,
                       selectcolor=INPUT_BG, activebackground=PANEL_BG).pack(side="left")
        tk.Label(toolbar, text="   \u2191 enviado (TX)   \u00b7   \u2193 recibido (RX)",
        ...
        self._log("Interfaz lista. Haga clic en \u25cf Conectar para abrir un puerto serial.", "info")
    ...
    def _build_right_panel(self, parent):
        ...
        panel = ttk.LabelFrame(parent, text="  Comando Personalizado  ", padding=(14, 10))
        panel.pack(fill="x", pady=(0, 10))
        ...
        tk.Button(cmd_frame, text="Enviar", font=("Segoe UI", 9, "bold"),
                  bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
        ...
        tk.Label(panel, text="Límites", font=("Segoe UI", 10, "bold"),
                 fg=ACCENT, bg=PANEL_BG).pack(anchor="w")
        tk.Label(panel, text="S0\u2013S3: 0\u2013180\u00b0\nS4 Pinza: 0\u201390\u00b0\nDirección del S2 invertida en el Arduino",
    ...
    def _build_arm_diagram(self, parent):
        ...
        """
        panel = ttk.LabelFrame(parent, text="  Diagrama de Estado del Brazo  ", padding=(10, 8))
        ...
        inner.pack(fill="x")
        tk.Label(inner, text="Control de los Servos", font=("Segoe UI", 11, "bold"),
        ...
        self._reset_all_btn = tk.Button(
            reset_row, text="\u21BA Reiniciar Todo", font=("Segoe UI", 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
    ...
    def _reset_all(self):
        ...
        self._log("todos los servos reiniciados a 90°", "info")
    ...
    def _arm_render_loop(self):
        try:
            ...
            try:
                self._log("error de renderizado del diagrama del brazo: " + str(exc), "err")
            except Exception:
                pass
    ...
    def _draw_arm(self):
        ...
        if self.connected:
            note = "ángulos objetivo (en vivo \u00b7 haga clic en Status para actualizar los valores reales)"
        else:
            note = "ángulos objetivo \u00b7 no conectado"
    ...
    def _build_traj_panel(self, parent):
        panel = ttk.LabelFrame(parent, text="  Trayectoria  ", padding=(14, 10))
        ...
        self.traj_record_btn = tk.Button(
            row1, text="\u25CF Grabar", font=("Segoe UI", 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
            relief="flat", padx=14, pady=6, cursor="hand2",
            command=self._toggle_record)
        self.traj_record_btn.pack(side="left")
        self.traj_play_btn = tk.Button(
            row1, text="\u25B6 Reproducir", font=("Segoe UI", 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
            relief="flat", padx=14, pady=6, cursor="hand2",
            command=self._toggle_play)
        self.traj_play_btn.pack(side="left", padx=(6, 0))
        tk.Button(row1, text="\u23F9 Detener", font=("Segoe UI", 11, "bold"),
                  bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=self._stop_play).pack(side="left", padx=(6, 0))
        tk.Button(row1, text="Limpiar", font=("Segoe UI", 11, "bold"),
                  bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
                  relief="flat", padx=14, pady=6, cursor="hand2",
        ...
        tk.Button(row2, text="\u2398 Capturar", font=("Segoe UI", 11, "bold"),
                  bg=GREEN, fg="#1e1e2e", activebackground=GREEN,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=lambda: self._snapshot(force=True)).pack(side="left")
        tk.Button(row2, text="\u21A1 Guardar...", font=("Segoe UI", 11, "bold"),
                  bg=GREEN, fg="#1e1e2e", activebackground=GREEN,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=self._save_trajectory).pack(side="left", padx=(6, 0))
        tk.Button(row2, text="\u21A2 Cargar...", font=("Segoe UI", 11, "bold"),
                  bg=GREEN, fg="#1e1e2e", activebackground=GREEN,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=self._load_trajectory).pack(side="left", padx=(6, 0))
        tk.Button(row2, text="Ejemplo predefinido", font=("Segoe UI", 11, "bold"),
        ...
        tk.Label(row3, text="Velocidad:", font=("Segoe UI", 9),
                 fg=SUBTEXT, bg=PANEL_BG).pack(side="left")
        ...
        tk.Label(row3, text="ms/cuadro", font=("Segoe UI", 8),
                 fg=SUBTEXT, bg=PANEL_BG).pack(side="left", padx=(4, 0))

        # Row 4: Loop checkbox
        row4 = tk.Frame(panel, bg=PANEL_BG); row4.pack(fill="x", pady=(4, 0))
        tk.Checkbutton(row4, text="Reproducir en bucle", variable=self.traj_loop,
        ...
        # Haga clic en cualquier lugar fuera de la lista para limpiar la
        # selección y liberar el modo en que la trayectoria "controla
        # los servos".
    ...
    def _toggle_record(self):
        if self.recording:
            self.recording = False
            self.traj_record_btn.configure(text="\u25CF Grabar", bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT)
            self._update_traj_status()
            self._log(f"grabación detenida -- {len(self.trajectory)} cuadros capturados", "info")
        else:
            self.trajectory = []
            self.recording = True
            self.traj_record_btn.configure(text="\u25A0 Detener", fg=TEXT)
            self._refresh_traj_list()
            self._update_traj_status()
            self._snapshot(force=True)
            self._log("grabación iniciada", "info")
    ...
    def _toggle_play(self):
        ...
        if not self.trajectory:
            self._log("nada que reproducir -- grabe o cargue una trayectoria primero", "warn")
            return
        self.playing = True
        self._suppress_snap = True
        self._play_index = 0
        self.traj_play_btn.configure(text="\u23F9 Pausar", fg=YELLOW)
        self._update_traj_status()
        self._log(f"reproducción iniciada -- {len(self.trajectory)} cuadros", "info")
        self._play_tick()
    ...
    def _stop_play(self):
        ...
        if self.traj_play_btn is not None:
            self.traj_play_btn.configure(text="\u25B6 Reproducir", bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT)
        self._update_traj_status()
        if was_playing:
            self._log("reproducción detenida", "info")
    ...
    def _play_tick(self):
        ...
        try:
            ...
            self._log(f"error de reproducción: {exc}", "err")
            self._stop_play()
            return
        ...
        try:
            ...
            self._log(f"error de envío en reproducción: {exc}", "err")
        try:
    ...
    def _clear_trajectory(self):
        ...
        self._refresh_traj_list()
        self._update_traj_status()
        self._log("trayectoria limpiada", "info")
    ...
    def _save_trajectory(self):
        if not self.trajectory:
            self._log("nada que guardar -- la trayectoria está vacía", "warn")
            return
        path = filedialog.asksaveasfilename(
            defaultextension=".json",
            filetypes=[("Trayectoria de servos", "*.json"), ("Todos los archivos", "*.*")],
        ...
        try:
            ...
            self._log(f"guardados {len(self.trajectory)} cuadros en {path}", "info")
        except Exception as exc:
            self._log(f"fallo al guardar: {exc}", "err")
    ...
    def _load_trajectory(self):
        ...
        try:
            ...
            self._log(f"fallo al cargar: {exc}", "err")
            return
        frames = payload.get("frames") if isinstance(payload, dict) else None
        if not frames or not isinstance(frames, list):
            self._log("el archivo no es una trayectoria válida (sin vector de cuadros)", "err")
            return
        if self.playing:
            self._stop_play()
        cleaned = []
        for f in frames:
            ...
            try:
                ...
                continue
        if not cleaned:
            self._log("ningún cuadro válido en el archivo", "err")
            return
        self.trajectory = cleaned
        self._play_index = 0
        self._refresh_traj_list()
        self._update_traj_status()
        self._log(f"cargados {len(cleaned)} cuadros de {path}", "info")
    ...
    def _load_example_trajectory(self):
        ...
        for f in example["frames"]:
            ...
            try:
                ...
                continue
        if not cleaned:
            self._log("la trayectoria de ejemplo incluida no tiene cuadros válidos", "err")
            return
        self.trajectory = cleaned
        self._play_index = 0
        self._refresh_traj_list()
        self._update_traj_status()
        self._log(f"ejemplo incluido cargado -- {len(cleaned)} cuadros", "info")
        self._toggle_play()
    ...
    def _on_traj_list_select(self, _event):
        ...
        for i, a in enumerate(frame):
            try:
                ...
                pass
        # Envía el comando para mover realmente los servos (maneja
        # "no conectado" con gracia y sincroniza target_vars + canvas).
        self._send_batch_and_sync(frame)
        self._log(
            "saltó al cuadro " + str(idx + 1) + "/" + str(len(self.trajectory))
            + ": " + " ".join(str(a) for a in frame),
            "info")

    def _on_traj_list_outside_click(self, event):
        """Haga clic en cualquier lugar fuera de la lista de trayectorias
        para limpiar la selección. Esto libera el modo en que el "cuadro
        controla los servos" para que mover los controles después no se
        interprete como saltar entre cuadros.
        """
    ...
    def _on_traj_list_edit(self, event):
        ...
        if self.playing or self.recording:
            self._log("no se puede editar un cuadro durante la reproducción o grabación", "warn")
            return
        sel = self.traj_list.curselection()
        if not sel:
            return
        ...
        def commit(_e=None):
            try:
                ...
                if _e is not None and getattr(_e, "widget", None) is top:
                    return
                txt = entry.get().strip()
                # Elimina el prefijo "N: " o "N:" si el usuario lo dejó
                if ':' in txt:
                    txt = txt.split(':', 1)[1].strip()
                parts = txt.split()
                if len(parts) != 5:
                    raise ValueError("se esperaban 5 valores de ángulo")
                new_angles = []
                ...
                try:
                    ...
                    pass
                self._log(f"cuadro {idx+1} editado -> {new_angles}", "info")
            except ValueError as exc:
                self._log(f"edición cancelada: {exc}", "warn")
            finally:
                try:
                    top.destroy()
                except Exception:
    ...
    def _update_traj_status(self):
        ...
        if self.playing:
            ...
            self.traj_status.set(f"\u25B6 Reproduciendo  {idx}/{n}")
        elif self.recording:
            self.traj_status.set(f"\u25CF  Grabando  ({n} cuadros)")
        else:
            self.traj_status.set(f"\u25CB  Inactivo  ({n} cuadros)")


    def _refresh_ports(self):
        ports = [p.device for p in list_ports.comports()]
        self.port_combo["values"] = ports if ports else ["(sin puertos)"]
        if ports:
    ...
    def _connect(self, port, baud):
        try:
            ...
            self.ser = None
            self._set_status(False, f"Fallo: {e}")
            self._log(f"Fallo en la conexión: {e}", "err")
            return
        ...
        self._set_status(True, f"Conectado {port} @ {baud}")
        self.conn_btn.configure(text="\u25cf  Desconectar")
        self._log(f"Conectado a {port} @ {baud}", "ok")
    ...
    def _send_command(self, cmd):
        ...
        if not self.connected or self.ser is None:
            self._log(f"[no conectado]  enviaría: {cmd}", "warn")
            return
        payload = (cmd.rstrip() + "\n").encode("utf-8")
        ser = self.ser
        with self._send_lock:
            try:
                ...
                self._log(f"Fallo al enviar: {e}", "err")
    ...
    def _on_rx_line(self, line):
        ...
        if m:
            ...
            if "ESTADO" in upper or "INIT OK" in upper:
                self.actual_angles = angles
                self._refresh_actual_display()
            elif "LOTE OK OBJETIVO" in upper:
                for i, ang in enumerate(angles):
                    self.target_vars[i].set(ang)
                self._log("  \u21b3 cuadro objetivo confirmado", "ok")
            ...
            try:
                if all(a == 90 for a in angles):
                    self._home_btn.configure(
                        text="\u2713  En casa (todos a 90°)",
                        bg=GREEN,
                    )
                else:
                    self._home_btn.configure(
                        text="\u2302  Inicio (todos a 90°)",
                        bg=ACCENT,
                    )
            except Exception:
                pass
    ...
    def _quick_action(self, btn, action):
        """Ejecuta una acción rápida y hace parpadear el botón para que el
        usuario vea la respuesta.  action en {"home", "status", "help"}.

        El orden importa: ejecutamos la acción PRIMERO para que el bg del
        botón ya esté actualizado (por ej. a "En casa" verde) antes de
        capturarlo como orig_bg, luego parpadeamos en amarillo encima y
        restauramos al nuevo estado. Así el parpadeo siempre se ve.
        """
        if action == "home":
