...
# Platform-aware UI font: Segoe UI ships with Windows; macOS uses Helvetica
# Neue (modern Mac default).  Both render Latin text cleanly; the Mac
# branch falls back gracefully if a specific weight is unavailable.
...
class ServoControlApp:
    ...
    def __init__(self):
        ...
        self._log("arm diagram auto-refresh started (every " + str(ARM_REFRESH_MS) + " ms)", "info")
    ...
    def _configure_styles(self):
        ...
        style.configure("TLabel",            background=BG, foreground=TEXT, font=(UI_FONT_FAMILY, 10))
        style.configure("Panel.TLabel",      background=PANEL_BG, foreground=TEXT, font=(UI_FONT_FAMILY, 10))
        style.configure("PanelMute.TLabel",  background=PANEL_BG, foreground=SUBTEXT, font=(UI_FONT_FAMILY, 9))
        style.configure("TButton",           font=(UI_FONT_FAMILY, 10))
        style.configure("Small.TButton",     font=(UI_FONT_FAMILY, 11), padding=(8, 4))
        style.configure("TCombobox",         font=(UI_FONT_FAMILY, 10))
        style.configure("TCheckbutton",      background=PANEL_BG, foreground=TEXT, font=(UI_FONT_FAMILY, 9), focuscolor=PANEL_BG)
        ...
        style.configure("TLabelframe.Label", background=PANEL_BG, foreground=ACCENT, font=(UI_FONT_FAMILY, 11, "bold"))
        style.configure("TSeparator",        background=SUBTEXT)

    def _build_ui(self):
        header = tk.Frame(self.root, bg=BG)
        header.pack(pady=(14, 4))
        tk.Label(header, text="Servo Arm Control",
                 font=(UI_FONT_FAMILY, 20, "bold"), fg=ACCENT, bg=BG).pack(side="left")
        tk.Label(header, text="  \u00b7  Arduino UNO R3  \u00b7  5\u00d7 MG995  \u00b7  Serial @ 9600",
                 font=(UI_FONT_FAMILY, 9), fg=SUBTEXT, bg=BG).pack(side="left", padx=8)
    ...
    def _build_connection_bar(self):
        ...
        tk.Label(inner, text="Connection", font=(UI_FONT_FAMILY, 10, "bold"),
                 fg=ACCENT, bg=PANEL_BG).pack(side="left", padx=(0, 12))

        tk.Label(inner, text="Port:", fg=TEXT, bg=PANEL_BG).pack(side="left")
        ...
        tk.Label(inner, text="  Baud:", fg=TEXT, bg=PANEL_BG).pack(side="left", padx=(8, 0))
        ...
        self.conn_btn = tk.Button(inner, text="\u25cf  Connect", font=(UI_FONT_FAMILY, 10, "bold"),
        ...
        self._status_lbl = tk.Label(inner, text="Disconnected", fg=SUBTEXT, bg=PANEL_BG, font=(UI_FONT_FAMILY, 9))
        self._status_lbl.pack(side="left")

    def _build_servo_panel(self, parent):
        panel = ttk.LabelFrame(parent, text="  Servo Control  ", padding=(14, 10))
        panel.pack(fill="x", pady=(0, 10))

        hdr = tk.Frame(panel, bg=PANEL_BG)
        hdr.pack(fill="x", pady=(0, 4))
        tk.Label(hdr, text="", width=22, bg=PANEL_BG).pack(side="left")
        tk.Label(hdr, text="Actual", font=(UI_FONT_FAMILY, 9, "bold"),
                 fg=GREEN, bg=PANEL_BG, width=6).pack(side="left")
        tk.Label(hdr, text="Target (slider / entry)",
                 font=(UI_FONT_FAMILY, 9, "bold"), fg=SUBTEXT, bg=PANEL_BG).pack(side="left", padx=(4, 0))
    ...
    def _build_log_panel(self, parent):
        panel = ttk.LabelFrame(parent, text="  Serial Log  ", padding=(10, 8))
        panel.pack(fill="x")
        ...
        tk.Checkbutton(toolbar, text="Auto-scroll", variable=self.autoscroll,
                       font=(UI_FONT_FAMILY, 9), fg=SUBTEXT, bg=PANEL_BG,
        ...
        self._log("GUI ready. Click \u25cf Connect to open a serial port.", "info")
    ...
    def _build_right_panel(self, parent):
        ...
        panel = ttk.LabelFrame(parent, text="  Custom Command  ", padding=(14, 10))
        panel.pack(fill="x", pady=(0, 10))
        ...
        tk.Button(cmd_frame, text="Send", font=(UI_FONT_FAMILY, 9, "bold"),
                  bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
    ...
    def _build_arm_diagram(self, parent):
        ...
        """
        panel = ttk.LabelFrame(parent, text="  Arm Status Diagram  ", padding=(10, 8))
        ...
        inner.pack(fill="x")
        tk.Label(inner, text="Servo Control", font=(UI_FONT_FAMILY, 11, "bold"),
        ...
        self._reset_all_btn = tk.Button(
            reset_row, text="\u21BA Reset All", font=(UI_FONT_FAMILY, 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
    ...
    def _reset_all(self):
        ...
        self._log("reset all servos to 90°", "info")
    ...
    def _arm_render_loop(self):
        try:
            ...
            try:
                self._log("arm diagram render error: " + str(exc), "err")
            except Exception:
                pass
    ...
    def _draw_arm(self):
        ...
        c.create_text(w / 2, h - 8, text=note, fill=SUBTEXT,
                      font=(UI_FONT_FAMILY, max(8, int(10 * SCALE)), "italic"))
    ...
    def _build_traj_panel(self, parent):
        panel = ttk.LabelFrame(parent, text="  Trajectory  ", padding=(14, 10))
        ...
        self.traj_record_btn = tk.Button(
            row1, text="\u25CF Record", font=(UI_FONT_FAMILY, 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
            relief="flat", padx=14, pady=6, cursor="hand2",
            command=self._toggle_record)
        self.traj_record_btn.pack(side="left")
        self.traj_play_btn = tk.Button(
            row1, text="\u25B6 Play", font=(UI_FONT_FAMILY, 11, "bold"),
            bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
            relief="flat", padx=14, pady=6, cursor="hand2",
            command=self._toggle_play)
        self.traj_play_btn.pack(side="left", padx=(6, 0))
        tk.Button(row1, text="\u23F9 Stop", font=(UI_FONT_FAMILY, 11, "bold"),
                  bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=self._stop_play).pack(side="left", padx=(6, 0))
        tk.Button(row1, text="Clear", font=(UI_FONT_FAMILY, 11, "bold"),
                  bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT,
                  relief="flat", padx=14, pady=6, cursor="hand2",
        ...
        tk.Button(row2, text="\u2398 Capture", font=(UI_FONT_FAMILY, 11, "bold"),
                  bg=GREEN, fg="#1e1e2e", activebackground=GREEN,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=lambda: self._snapshot(force=True)).pack(side="left")
        tk.Button(row2, text="\u21A1 Save...", font=(UI_FONT_FAMILY, 11, "bold"),
                  bg=GREEN, fg="#1e1e2e", activebackground=GREEN,
                  relief="flat", padx=14, pady=6, cursor="hand2",
                  command=self._save_trajectory).pack(side="left", padx=(6, 0))
        tk.Button(row2, text="\u21A2 Load...", font=(UI_FONT_FAMILY, 11, "bold"),
                  bg=GREEN, fg="#1e1e2e", activebackground=GREEN,
                  relief="flat", padx=14, pady=6, cursor="hand2",
        ...
        tk.Label(row3, text="Speed:", font=(UI_FONT_FAMILY, 9),
                 fg=SUBTEXT, bg=PANEL_BG).pack(side="left")
        ...
        tk.Label(row3, text="ms/frame", font=(UI_FONT_FAMILY, 8),
                 fg=SUBTEXT, bg=PANEL_BG).pack(side="left", padx=(4, 0))

        # Row 4: Loop checkbox
        row4 = tk.Frame(panel, bg=PANEL_BG); row4.pack(fill="x", pady=(4, 0))
        tk.Checkbutton(row4, text="Loop playback", variable=self.traj_loop,
                       font=(UI_FONT_FAMILY, 9), fg=TEXT, bg=PANEL_BG,
                       selectcolor=INPUT_BG, activebackground=PANEL_BG).pack(side="left")
    ...
    def _toggle_record(self):
        if self.recording:
            self.recording = False
            self.traj_record_btn.configure(text="\u25CF Record", bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT)
            self._update_traj_status()
            self._log(f"recording stopped -- {len(self.trajectory)} frames captured", "info")
        else:
            self.trajectory = []
            self.recording = True
            self.traj_record_btn.configure(text="\u25A0 Stop", fg=TEXT)
            self._refresh_traj_list()
            self._update_traj_status()
            self._snapshot(force=True)
            self._log("recording started", "info")
    ...
    def _toggle_play(self):
        ...
        if not self.trajectory:
            self._log("nothing to play -- record or load a trajectory first", "warn")
            return
        self.playing = True
        self._suppress_snap = True
        self._play_index = 0
        self.traj_play_btn.configure(text="\u23F9 Pause", fg=YELLOW)
        self._update_traj_status()
    ...
    def _stop_play(self):
        ...
        if self.traj_play_btn is not None:
            self.traj_play_btn.configure(text="\u25B6 Play", bg=ACCENT, fg="#1e1e2e", activebackground=ACCENT)
        self._update_traj_status()
        if was_playing:
            self._log("playback stopped", "info")
    ...
    def _clear_trajectory(self):
        ...
        self._refresh_traj_list()
        self._update_traj_status()
        self._log("trajectory cleared", "info")
    ...
    def _save_trajectory(self):
        if not self.trajectory:
            self._log("nothing to save -- trajectory is empty", "warn")
            return
    ...
    def _load_trajectory(self):
        ...
        if not frames or not isinstance(frames, list):
            self._log("file is not a valid trajectory (no frames array)", "err")
            return
        if self.playing:
            self._stop_play()
        cleaned = []
        for f in frames:
            ...
            try:
                ...
                continue
        if not cleaned:
            self._log("no valid frames in file", "err")
            return
    ...
    def _on_traj_list_edit(self, event):
        ...
        if self.playing or self.recording:
            self._log("cannot edit frame while playing or recording", "warn")
            return
        sel = self.traj_list.curselection()
        if not sel:
            return
    ...
    def _on_rx_line(self, line):
        ...
        if m:
            ...
            if "STATUS" in upper or "INIT OK" in upper:
                ...
                self._log("  \u21b3 target frame confirmed", "ok")
